                      ...
                      <div key={i} className="flex items-center justify-between p-4 bg-slate-950/50 rounded-2xl border border-slate-800 hover:border-slate-700 transition-colors">
                        ...
                        <div className="flex flex-col items-end gap-1">
                          <span className={`text-[10px] font-black px-2 py-0.5 rounded border ${
                            service.risk === 'CRITICAL' ? 'bg-red-500/10 text-red-500 border-red-500/20' : 
          ...
          {activeTab === 'hardening' && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 space-y-6">
              <div className="flex justify-between items-end mb-4">
                <div>
                  <h3 className="text-2xl font-black tracking-tight">System Defense Console</h3>
                  <p className="text-slate-400 text-sm mt-1">Select mitigations to harden your system configuration.</p>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {mitigations.map((m) => (
                  <div key={m.id} className={`p-6 rounded-3xl border transition-all duration-300 flex flex-col ${
                    m.isApplied ? 'bg-emerald-500/5 border-emerald-500/30 ring-1 ring-emerald-500/10' : 'bg-slate-900/40 border-slate-800'
                  }`}>
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${
                        m.riskLevel === 'CRITICAL' ? 'bg-red-500/10 text-red-500 border-red-500/20' : 
...
const NavItem: React.FC<{ active: boolean; onClick: () => void; icon: React.ReactNode; label: string }> = ({ active, onClick, icon, label }) => (
  <button 
    ...
    className={`w-full flex items-center gap-4 px-5 py-3.5 rounded-2xl transition-all ${
      active ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-900/40 ring-1 ring-indigo-500' : 'text-slate-500 hover:text-slate-200 hover:bg-slate-800/40'
    }`}
  >
    {icon}
